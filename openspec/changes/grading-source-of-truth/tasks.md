...
- [x] 4.1 Add `ICompetencyMastery { levelCode, overall: IMasteryCheck, competencies: ICompetencyMasteryItem[] }`, `IMasteryCheck { minScore, percentage, achieved }`, `ICompetencyMasteryItem extends IMasteryCheck { competency }` to both `IExamResult` copies — **lockstep checklist item**.
- [x] 4.2 Implement `computeMastery(competencyScores, level)` in `scoring.ts`: per-competency and overall achieved/not-achieved vs `level.competencyRequirements[].minScore`/`overallMinScore`; omit for placement, unresolved/inactive `targetLevel`, or no `competencyRequirements`. *(level-mastery-indicator: Per-Competency Mastery Indicator; Graceful Absence of Level Data — all 3 scenarios)*
- [x] 4.3 Wire `grade-exam.ts`: resolve `exam.targetLevel` → `getLevels().findOne({code, isActive:true})`; derive per-competency scores from question results (works for sectioned and flat-pool exams alike); call `computeMastery`; store `competencyMastery` without reading it back into the already-computed `passed`. *(Mastery Indicator Does Not Affect Pass/Fail — both scenarios; Flat-question-pool scenario)*
- [x] 4.4 Update `exam-service/src/utils/resultVisibility.ts` so `competencyMastery` appears in the full view and is stripped when `resultsHidden`.
- [x] 4.5 Update `frontend` `StudentResults.tsx` and the admin review screen to render `competencyMastery` badges when present.
- [x] 4.6 Extend `scripts/e2e/grading-pipeline.e2e.js`: upsert a level with `competencyRequirements`; assert `competencyMastery` present with correct achieved/not-achieved; assert `passed` unchanged when mastery fails but score passes (and vice versa); assert mastery omitted for a placement exam. Verify: docker e2e run.
...
**Review fixes (PR 4)**:
- Mastery is skipped while a result is `pending_ai_review`, because pending questions count as 0.
- A failed Level lookup omits the indicator instead of failing grading.
- Levels are matched with `isActive != false`.
- The panel tolerates missing `competencies`.
- StudentResults clears the previous result's detail before loading the next one.

**Known limitation**: `/regrade-session` only rewrites results whose auto-graded scores changed. It does not backfill mastery for older results, and it does not refresh mastery after a Level's minimums are edited. `/regrade-attempt` (force) does both.

**Landed (chain/32-level-mastery)**: 6/6 tasks complete, verified working end to end, **NOT yet committed** (working tree only). `git diff --stat` on tracked files: 14 files, 322 insertions(+)/13 deletions(-) = 335 changed lines; plus 2 untracked files (`mcp-grading-server/tests/mastery-scoring.test.ts` 122, `frontend/src/components/exam-review/CompetencyMasteryPanel.tsx` 93 = 215 lines) = **550 changed lines total** — over the design.md estimate (~200) and over the 400-line review budget. This is the 4th PR in this change to exceed its design.md estimate (same pattern as PR 2a/2b/3): a "make X visible everywhere" feature touches every layer (level lookup + pure scoring + grade-exam + regrade-session + Mongoose lockstep + resultVisibility docs/tests + 2 frontend surfaces + e2e), which is inherently wider than a single-service estimate suggests. **Orchestrator/user decision needed before commit** (per `ask-on-risk`, nothing committed yet so the decision can still be made): accept as-is (this is the full, already-verified PR 4 scope; it is also the LAST PR in the chain, so there is no "next PR" to push a split into) — or split along the natural seam: (1) backend — level resolution + `computeMastery` + `grade-exam.ts`/`regrade-session` wiring + Mongoose lockstep + `resultVisibility.ts` doc/tests + `mastery-scoring.test.ts`, ≈347 lines, and (2) frontend — `CompetencyMasteryPanel.tsx` + the two screens + types + e2e Phase 5, ≈203 lines.

Beyond the six tasks:
- `/regrade-session` (`grading.routes.ts`) now batch-fetches active levels once per session and recomputes `competencyMastery` via the same `computeMastery` used by `grade-exam.ts`, and `competencyMastery` joined `UNSETTABLE_GRADED_FIELDS` so a stale mastery verdict is `$unset`, not left behind, when a regrade no longer resolves one.
- **Gotcha (documented, not fixed — pre-existing, shared by `passed`/`passingScore`/`sections`)**: neither service's Mongo connection sets `ignoreUndefined: true`, so on a fresh `insertOne` an `undefined` field (mastery omitted) is stored as BSON `null`, not absent. Only a later `$unset` (via `buildUnsetForUndefined`, e.g. on `/regrade-session`) truly removes the key. `computeMastery`'s callers and the e2e assertions treat `null`/`undefined` as equivalent (`== null`); no runtime impact on `resultVisibility.ts`'s truthy checks or the frontend panel's `!mastery` guard.
- `getLevels()` is now typed `Collection<ILevel>` (was untyped `Collection`); `ILevel` also carries the optional `description`/`competencyRequirements[].description`/`canDoStatements` fields `generate-question.ts` already reads, so no other call site needed a cast.
- Frontend "No evaluado" (a level-required competency the exam never evaluated) needed no new backend field: `CompetencyMasteryPanel` diffs the returned `competencies` list against a fixed 6-competency set (every `Level` document requires exactly those 6 keys — verified live against `cba_platform.levels`).
- `resultVisibility.ts` needed no code change for the allowlist behavior itself (`toStudentView` already returns the full `result` unchanged when visible, and the hidden/pending minimal shape already excludes anything not in `CARRY_OVER_FIELDS`) — only a documentation comment plus 2 explicit regression tests.
- Scoped out (documented, not a task 4.5 requirement): `frontend/src/modules/admin/components/student-history/ExamDetailDialog.tsx` (a secondary admin/student-history dialog sharing `AdminExamResultDetail`) gets the `competencyMastery` data through the type but does not render the panel — budget/scope call, not a gap in the primary teacher/admin review screen (`ResultDetailDialog.tsx`).
- The internal `GradeExamResponse` HTTP schema (`grading.schemas.ts`) was NOT extended with `competencyMastery` — nothing in the assigned frontend surfaces (student detail, admin review) consumes that synchronous HTTP response; both read the stored `exam_results` document via exam-service's own endpoints.

Verified: `mcp-grading-server` tsc clean + `npm test` 64/64 node:test cases (10 new in `mastery-scoring.test.ts`); `exam-service` tsc clean + jest 10/10 suites, 95/95 tests (2 new in `resultVisibility.test.ts`); `frontend` `tsc -b` clean + `npm run build` clean (pre-existing chunk-size warning only) + eslint diffed against a `git stash`d baseline — 23/23 problems identical before and after on every touched pre-existing file, and 0 problems on the new `CompetencyMasteryPanel.tsx`. Live: rebuilt `grading-service` and `exam-service` Docker images (both healthy; `@cba/events` EINTEGRITY workaround applied and reverted for both, per the now-3-times-documented pattern) and ran the extended `scripts/e2e/grading-pipeline.e2e.js` — **48/51 checks passed**; the 3 failures are Phase 3/4 (`showResults` email/in-app gating, PR 3 scope) and are caused by the currently-running `notifications-service` container being built from the security branch, which predates the `showResults` feature (confirmed via `grep showResults` on its compiled `dist/` — 0 matches); not a PR 4 regression. All 9 new Phase 5 (mastery) checks passed. Re-ran `scripts/e2e/ai-grading.e2e.js` unmodified — 40/40 checks passed, no regression.

---

## Cross-PR Verification (run once all PRs land)

- [x] Grep the 5 consumers for `>= 60` / `>= 70` / `passingScore: 70` outside the 3 documented default-70 constants. Only the previously-documented residual UI quality-tier literals remain (`exam-result-pdf.service.ts` `getPerformanceLevel`-style tiers, `examResultService.ts` `generateFeedback`/`getScoreColor`) — not the `passed` verdict, flagged as out-of-scope since task 1b.11.
- [x] Diff `mcp-grading-server/src/types/index.ts` vs `exam-service/src/models/examResult.model.ts` field-by-field — lockstep confirmed (including the new mastery fields).
- [x] Run all 6 proposal Success Criteria checks end to end via `scripts/e2e/{grading-pipeline,ai-grading}.e2e.js` — see PR 4's "Landed" note above for the live run and the pre-existing (non-PR-4) Phase 3/4 caveat.
