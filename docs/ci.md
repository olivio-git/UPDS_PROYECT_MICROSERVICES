...
Right after that, an **"Install e2e-only npm deps (socket.io-client)"** step runs `npm install socket.io-client@4 --no-save` inside the already-running `exam-service` container. Three scripts (`kick-and-push.e2e.js`, `lockdown-infractions.e2e.js`, `exam-lifecycle.e2e.js`) connect to `notifications-service` over Socket.IO to assert real-time push behavior, but `socket.io-client` is deliberately not a dependency of `exam-service`'s own `package.json` (it's test-only) — each of those scripts' header comments already documented installing it this way locally; the workflow just automates it once, on top of the built image, before any script that needs it runs.
