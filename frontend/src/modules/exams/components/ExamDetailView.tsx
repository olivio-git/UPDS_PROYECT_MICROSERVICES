    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
            variant="outline"
            onClick={onBack}
            className="px-3 py-2 bg-dark-light border border-line rounded-lg text-white hover:bg-dark-light/80 flex items-center gap-2"
          >
            <ArrowLeft className="w-4 h-4" />
            Volver
          </Button>
          <div>
            <h2 className="text-xl font-semibold text-white">Detalles del examen</h2>
            <p className="text-sm text-gray-400">ID: {exam.name}</p>
          </div>
          </div>
      ...
      <div className="bg-box border border-line rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Nivel Objetivo</label>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`inline-flex px-3 py-1 text-sm font-medium border rounded-lg ${
                ...
                {/* 
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Puntaje Mínimo</label>
            <div className="flex items-center gap-2">
              <Award className="w-4 h-4 text-yellow-400" />
              <span className="text-white font-medium">{exam.structure.passingScore}%</span>
            </div>
          </div>
              */}
        </div>
        ...
        {exam.description && (
          <div className="mt-6">
            ...
            <p className="text-gray-300 leading-relaxed">{exam.description}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-box border border-line rounded-xl p-6">
        ...
        <div className="space-y-4">
          {exam.structure.sections?.map((section, index) => (
            <div key={section.id} className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-medium text-gray-200">{section.name}</h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-white font-medium">{section.questionCount}</p>
                </div>
                <div>
                  <span className="text-gray-400">Puntos:</span>
                  <p className="text-white font-medium">{section.points}</p>
                </div>
                <div>
                  <span className="text-gray-400">Duración:</span>
                  <p className="text-white font-medium">{formatDuration(section.duration || 0)}</p>
                </div>
                <div>
              ...
              {section.instructions && (
                <div className="mt-3 pt-3 border-t border-gray-700">
                  ...
                  <p className="text-gray-300 text-sm mt-1">{section.instructions}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="bg-box border border-line rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Aleatorizar preguntas</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Aleatorizar opciones</span>
              <span className={`px-2 py-1 text-xs rounded ${
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Mostrar resultados</span>
              <span className={`px-2 py-1 text-xs rounded ${
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Permitir revisión</span>
              <span className={`px-2 py-1 text-xs rounded ${
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Máximo de intentos</span>
      ...
      {(exam.createdAt || exam.updatedAt) && (
        <div className="bg-box border border-line rounded-xl p-6">
          <h2 className="text-lg font-semibold text-gray-200 mb-4">Metadatos</h2>
