...
const QuestionMultimedia: React.FC<Props> = ({
  ...
  const handleRecordingComplete = useCallback((audioBlob: Blob, audioUrl: string) => {
    try {
      ...
      // Debug del archivo creado
      debugFile(audioFile, 'Recorded Audio File');
  ...
  // Manejar selección de archivo de audio mejorada
  const handleAudioFileSelect = useCallback((file: File) => {
    try {
      debugFile(file, 'Selected Audio File');
      ...
      toast.success(`Audio seleccionado: ${file.name} (${formatFileSize(file.size)})`);
    } catch (error) {
      console.error('Error al procesar archivo de audio:', error);
      toast.error('Error al procesar el archivo de audio');
    }
  }, [onAudioChange, recordedAudioUrl]);

  // Manejar selección de archivo de imagen mejorada
  const handleImageFileSelect = useCallback((file: File) => {
    try {
      debugFile(file, 'Selected Image File');
      
      onImageChange(file);
      
      toast.success(`Imagen seleccionada: ${file.name} (${formatFileSize(file.size)})`);
    } catch (error) {
      console.error('Error al procesar archivo de imagen:', error);
      toast.error('Error al procesar el archivo de imagen');
    }
  }, [onImageChange]);

  // Crear handlers con validación
  const audioUploadHandler = createFileUploadHandler(
    AUDIO_FILE_OPTIONS,
    handleAudioFileSelect,
    (error) => console.error('Audio validation error:', error)
  );

  const imageUploadHandler = createFileUploadHandler(
    IMAGE_FILE_OPTIONS,
    handleImageFileSelect,
    (error) => console.error('Image validation error:', error)
  );
          ...
          <div className="space-y-4">
            ...
            {(audioFile || recordedAudioUrl) && (
              <div className="mb-4">
                ...
                <AudioPlayer
                  src={recordedAudioUrl || (audioFile ? createManagedURL(audioFile) : '')}
                  variant="compact"
